...
const float d4_coeffs[] = {
    0.4829629131445341f,  // h0
    0.8365163037378079f,  // h1
    0.2241438680420134f,  // h2
    -0.1294095225512604f  // h3
};
// Low pass decomposition filter
const float lpd[] = {
    d4_coeffs[0], d4_coeffs[1], d4_coeffs[2], d4_coeffs[3]
};

// High pass decomposition filter
const float hpd[] = {
    d4_coeffs[3], -d4_coeffs[2], d4_coeffs[1], -d4_coeffs[0]
};

// Low pass reconstruction filter
const float lpr[] = {
    d4_coeffs[2], d4_coeffs[1], d4_coeffs[0], d4_coeffs[3]
};

// High pass reconstruction filter
const float hpr[] = {
    d4_coeffs[3], -d4_coeffs[0], d4_coeffs[1], -d4_coeffs[2]
};
void daubechies_decompose_1d(float* data, int length) {
    if (length < 4) return;  // Minimum length for D4 wavelet
    
    float* temp = (float*)malloc(length * sizeof(float));
    int half = length >> 1;
    
    // Apply decomposition filters
    for (int i = 0; i < half; i++) {
        int j = i << 1;
        temp[i] = 0;  // Approximation coefficient
        temp[i + half] = 0;  // Detail coefficient
        
        // Convolve with filter coefficients
        for (int k = 0; k < 4; k++) {
            int idx = (j + k) % length;  // Periodic boundary handling
            temp[i] += data[idx] * lpd[k];
            temp[i + half] += data[idx] * hpd[k];
        }
    }
...
// Replace haar_reconstruct_1d with daubechies_reconstruct_1d
void daubechies_reconstruct_1d(float* data, int length) {
    if (length < 4) return;
    ...
    for (int i = 0; i < half; i++) {
        for (int k = 0; k < 4; k++) {
            int idx = (i << 1) + k;
            if (idx >= length) idx -= length;  // Periodic boundary handling
            
            // Reconstruct using both approximation and detail coefficients
            temp[idx] += data[i] * lpr[k] + data[i + half] * hpr[k];
        }
    }
...
// Modify the haar_transform_2d function to use Daubechies wavelets
void daubechies_transform_2d(WaveletTransform* wt, float** input) {
    ...
    for (int level = 0; level < wt->levels && current_width > 4 && current_height > 4; level++) {
        // Transform rows
        for (int i = 0; i < current_height; i++) {
            memcpy(row, wt->coefficients[i], current_width * sizeof(float));
            daubechies_decompose_1d(row, current_width);
        ...
        for (int j = 0; j < current_width; j++) {
            ...
            daubechies_decompose_1d(col, current_height);
            for (int i = 0; i < current_height; i++) {
...
// Modify the haar_inverse_transform_2d function to use Daubechies wavelets
void daubechies_inverse_transform_2d(WaveletTransform* wt) {
    float* row = (float*)malloc(wt->width * sizeof(float));
    float* col = (float*)malloc(wt->height * sizeof(float));
    ...
    for (int level = wt->levels - 1; level >= 0; level--) {
        ...
        for (int j = 0; j < current_width; j++) {
            ...
            daubechies_reconstruct_1d(col, current_height);
            for (int i = 0; i < current_height; i++) {
                wt->coefficients[i][j] = col[i];
            }
        }
        ...
        for (int i = 0; i < current_height; i++) {
            memcpy(row, wt->coefficients[i], current_width * sizeof(float));
            daubechies_reconstruct_1d(row, current_width);
            memcpy(wt->coefficients[i], row, current_width * sizeof(float));
        }
...
// Modify the apply_wavelet_compression function to use Daubechies wavelets
float** apply_wavelet_compression(float** input, int width, int height, int levels, float threshold) {
    ...
    // Forward transform using Daubechies wavelets
    daubechies_transform_2d(wt, input);
    
    // Apply threshold for compression
    apply_wavelet_threshold(wt, threshold);
    
    // Inverse transform using Daubechies wavelets
    daubechies_inverse_transform_2d(wt);
...
int main(int argc, char **argv) {
    ...
    //Compute compression
    float** filteredY = apply_wavelet_compression(Y_array,width,height,3,compression_rate);
    float** filteredCb = apply_wavelet_compression(Cb,width,height,3,compression_rate/4);
    float** filteredCr = apply_wavelet_compression(Cr,width,height,3,compression_rate/4);
