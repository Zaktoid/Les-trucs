...
float** apply_wavelet_compression(float** input, int width, int height, int levels, float threshold) {
    ...
    // Count zeroes before thresholding
    int initial_zeros = 0;
    for (int i = 0; i < wt->height; i++) {
        for (int j = 0; j < wt->width; j++) {
            if (wt->coefficients[i][j] == 0.0f) {
                initial_zeros++;
            }
        }
    }
    ...
    // Count zeroes after thresholding and print statistics
    int final_zeros = 0;
    for (int i = 0; i < wt->height; i++) {
        for (int j = 0; j < wt->width; j++) {
            if (wt->coefficients[i][j] == 0.0f) {
    ...
    int total_coefficients = wt->width * wt->height;
    int zeroed_by_compression = final_zeros - initial_zeros;
    float zero_percentage = (float)zeroed_by_compression / total_coefficients * 100.0f;
    
    printf("Wavelet Coefficient Statistics:\n");
    printf("Total Coefficients: %d\n", total_coefficients);
    printf("Coefficients zeroed by compression: %d\n", zeroed_by_compression);
    printf("Percentage of coefficients zeroed: %.2f%%\n", zero_percentage);
    printf("Threshold value used: %.4f\n\n", threshold);
        ...
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                ...
                if (imgY < height && imgY >= 0 && imgX >= 0 && imgX < width) {
                    ...
                    SDL_SetRenderDrawColor(rendu, coeff_val*10, coeff_val*10, coeff_val*10, 255);
